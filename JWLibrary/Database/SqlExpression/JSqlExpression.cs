...
namespace JWLibrary.Database {
    public class JSqlExpression {
        ...
        public JSqlExpression<Ta, Tb> Join<Ta, Tb>(Expression<Func<Ta, Tb, object>> predicate) where Ta : class where Tb : class {
    ...
    public partial class JSqlExpression<T> where T : class {
        ...
        public JSqlExpression<T> Join<Ta, Tb>(Expression<Func<Ta, Tb, object>> predicate) where Ta : class where Tb : class {
        ...
        public JSqlExpression<T> And(Expression<Func<T, bool>> predicate) {
            ...
            if (body.NodeType == ExpressionType.Call) {
                ...
                if (commandNodeType == ExpressionType.Constant) {
                    ...
                    sql.Append(SqlExpressionUtil.SqlIn(args));
                }
            }
            else {
        ...
        public IEnumerable<T> Query(ENUM_DATABASE_TYPE type) {
            ...
            JDatabaseResolver.Resolve(type).DbExecute((c, t) => {
                result = c.Query<T>(this.Build(), _parameters.Select(m => m.Value));
            });
            return result;
        }
    }
    ...
    public class JSqlExpression<Ta, Tb> where Ta : class where Tb : class {
        ...
        private XList<string> _onAnds = new XList<string>();
        private XList<string> _wheres = new XList<string>();
            ...
            if (body.NodeType == ExpressionType.Convert) {
                if (body.Operand.NodeType == ExpressionType.Equal) {
                    ...
                    if (commandNodeType == ExpressionType.Constant) {
                        ...
                        xsb.Append(SqlExpressionUtil.SqlIn(args));
                    }
                    var sql = string.Empty;
                    xsb.Release(out sql);
                    _onAnds.Add(sql);                    
        ...
        public string Build() {
            ...
            _onAnds.xForEach(_onAnd => {
                xsb.AppendLine(_onAnd);
            });
            _wheres.xForEach(_where => {
                xsb.AppendLine(_where);
            });
