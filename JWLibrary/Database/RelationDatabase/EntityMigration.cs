...
namespace JWLibrary.Database
{
    ...
    public class EntityMigration<TEntity> : IEntityMigration<TEntity>
        where TEntity : class
    {
        private readonly List<string> _columnExpressions = new();
        ...
        public string Build()
        {
            ...
            if (existsTableColumns.xIsNotEmpty()) return Scenario1(existsTableColumns);
        ...
        {
            _backupExpression =
                $"SELECT {string.Join(",", existsColumns)} INTO {_tableName}_BACKUP FROM [{_tableName}]";
            _dropExpression = $"DROP TABLE IF EXISTS [{_tableName}]";
            _createExpression = CreateTable();
            _backupInsertExpression =
                $"INSERT INTO [{_tableName}] ({string.Join(",", existsColumns)}) SELECT {string.Join(",", existsColumns)} FROM [{_tableName}_BACKUP]";
        ...
        private string CreateTable()
        {
            ...
            _columnExpressions.xForEach(",", (item, split) => { sb.AppendLine($"{item}{split}"); });
