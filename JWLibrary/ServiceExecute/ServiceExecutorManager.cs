...
namespace JWLibrary.ServiceExecutor
{
        ...
        where TIService : IServiceBase
    {
        ...
        public ServiceExecutorManager<TIService> SetRequest<TRequest>(TRequest request,
            Action<TIService, TRequest> action)
        {
        ...
        public async Task<bool> OnExecutedAsync(Func<TIService, Task<bool>> func)
        {
            ...
            if (service.Validate())
            {
                ...
                if (executed.xIsFalse()) throw new Exception($"{typeof(TIService).Name} executed failed.");
                return executed;
            }
