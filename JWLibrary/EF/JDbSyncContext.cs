...
namespace JWLibrary.EF
{
    ...
    public class JDbSyncContext : IJDbSyncContext
    {
        ...
        public void Dispose()
        {
            ...
            _destDbContexts.xForEach(context => { context.Dispose(); });
        }
        ...
        private void BeginTrasaction()
        {
            ...
            _destDbContexts.xForEach(context => { context.Database.BeginTransaction(IsolationLevel.ReadUncommitted); });
        }

        private void CommitTransaction()
        {
            _srcDbContext.Database.CommitTransaction();
            _destDbContexts.xForEach(context => { context.Database.CommitTransaction(); });
        }

        private void RollbackTransaction()
        {
            _srcDbContext.Database.RollbackTransaction();
            _destDbContexts.xForEach(context => { context.Database.RollbackTransaction(); });
        }
    }
}